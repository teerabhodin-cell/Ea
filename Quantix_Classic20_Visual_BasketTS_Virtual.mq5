...
//|        [OPTIMIZED: Strict Session & OrderSendAsync Warning Patch]|
//|        [FIXED: Breakeven trigger, Equity Lock vs DD-Stop,        |
//|                Buy/Sell gap-anchor cross-contamination,          |
//|                Lot volume step/min/max normalization]            |
//+------------------------------------------------------------------+
#property strict
...
bool     BreakevenActivated   = false; // latches once profit crosses BreakevenTriggerUSD
...
bool IsMarketOpen()
{
   ...
   if(SymbolInfoSessionTrade(_Symbol, (ENUM_DAY_OF_WEEK)dt.day_of_week, 0, from, to))
   {
      ...
      int fromMins    = dtFrom.hour * 60 + dtFrom.min;
      int toMins      = dtTo.hour * 60 + dtTo.min;
...
//| TRADE_RETCODE_INVALID_VOLUME on brokers whose step isn't 0.01.   |
//+------------------------------------------------------------------+
double GetCalculatedLotSize(int nextLevel)
{
   double base = BaseLot;
...
//| tick, which is unreachable whenever Lock < Trigger (the normal   |
//| configuration), so breakeven never closed anything.              |
//+------------------------------------------------------------------+
void ApplyBasketBreakevenAndPartial(double currentProfit)
{
   if(!IsMarketOpen()) return;
...
//+------------------------------------------------------------------+
//| Expert Tick                                                      |
//| FIXED: UseEquityLock no longer short-circuits the whole tick.    |
//| Per the input's own description ("หยุดเปิดไม้ใหม่" / stop opening |
//| new orders), it must only block ExecuteGridLogic() below -       |
//| UpdateDrawdownTracker() (Max DD emergency stop), the basket       |
...
void OnTick()
{
   ...
   if(!timeAllowed && openPositions > 0 && !IsClosingState)
   {
      if(currentProfit > 0)
      {
         IsClosingState = true;
...
//+------------------------------------------------------------------+
//| Delete All Pending Orders Function (PATCHED OrderSendAsync)      |
//+------------------------------------------------------------------+
void DeleteAllPendingOrders()
{
   if(!IsMarketOpen()) return;
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(ticket > 0 && OrderSelect(ticket))
      {
         if(OrderGetString(ORDER_SYMBOL) == _Symbol && OrderGetInteger(ORDER_MAGIC) == MagicNumber)
         {
            ...
            {
               Print("Failed to delete pending order via Async: ", ticket);
            }
         }
      }
   }
...
void PlacePendingGridServer()
{
   if(IsClosingState || !IsMarketOpen()) return;
   ...
   for(int level = 1; level <= TotalLevels; level++)
   {
      ...
      if(canBuyFilter)
      {
         ...
         if(!OrderSend(request, result))
         {
            if(result.retcode == TRADE_RETCODE_MARKET_CLOSED || result.retcode == TRADE_RETCODE_TRADE_DISABLED) break;
         }
      }
...
void CheckAndExecuteVirtualGrid()
{
   ...
   if(buyCount < TotalLevels && canBuyFilters)
   {
      ...
      if(canSendBuy && SymbolInfoInteger(_Symbol, SYMBOL_SPREAD) <= adjSpread)
      {
         ...
         {
            if(result.retcode == TRADE_RETCODE_MARKET_CLOSED || result.retcode == TRADE_RETCODE_TRADE_DISABLED) return;
         }
      }
...
//+------------------------------------------------------------------+
//| Synchronous Close All (Fixed Magic Match Check)                  |
//+------------------------------------------------------------------+
void ManualCloseAllSync()
{
   MqlTradeRequest request;
   ...
   for(int i = OrdersTotal() - 1; i >= 0; i--)
   {
      ...
      if(ticket > 0 && OrderSelect(ticket))
      {
         if(OrderGetString(ORDER_SYMBOL) == _Symbol && OrderGetInteger(ORDER_MAGIC) == MagicNumber)
         {
            ...
            if(!OrderSendAsync(request, result))
            {
               Print("Async cancel order failed for order: ", ticket);
            }
         }
      }
   }
   ...
   while(retryCount < 5)
   {
      for(int i = PositionsTotal() - 1; i >= 0; i--)
      {
         ...
         if(!OrderSend(request, result))
         {
            if(result.retcode == TRADE_RETCODE_MARKET_CLOSED || result.retcode == TRADE_RETCODE_TRADE_DISABLED)
            {
               break;
            }
         }
...
void UpdateDashboard(double currentProfit, double maxProfit, double currentTS, int openPos, int pendingOrders)
{
   ...
   if(!marketOpen) {
      ...
      ObjectSetString(0, UI_PREFIX+"LED_Text", OBJPROP_TEXT, GetUIString("ตลาดปิดทำการ (MARKET CLOSED)", "MARKET CLOSED"));
      ...
      ObjectSetInteger(0, UI_PREFIX+"LED_Icon", OBJPROP_COLOR, clrRed);
      ObjectSetString(0, UI_PREFIX+"LED_Text", OBJPROP_TEXT, GetUIString("นอกเวลาเทรด (รอเคลียร์ไม้เมื่อมีกำไร)", "OFF-TIME / HOLD FOR PROFIT"));
