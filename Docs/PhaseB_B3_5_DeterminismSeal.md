...
**Status: SEALED (2026-08-14).** All 7 criteria verified, including a
cross-session check across a genuine MT5 restart. See
`Docs/PhaseB_B3_DataHubDeterminism.md` for B3 itself.

## Restart evidence (the criterion that closed this out)

A fresh EA startup at 16:18:33 (`MLQuantAI v0.1.0 starting`, event store
replayed cleanly: `58 lines, health=HEALTHY`, `replay: 3 lifecycle events
applied, 0 failed, 56 system events applied`) preceded a
`MLQuantAI_Test_DataHubDeterminism` run at 16:18:46 that hit:

```
[PASS] rebuilt context_hash for the SAME anchor bar matches the hash a
PREVIOUS run of this script persisted
```

This is a real process-boundary crossing (not just a script
re-invocation within an already-running terminal), for the same M5
anchor bar as the fixture-writing run before it - the specific gap the
16:07/16:10 runs left open.
