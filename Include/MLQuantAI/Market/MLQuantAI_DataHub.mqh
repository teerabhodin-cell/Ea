...
// Scans M15 bars from today's broker-server-time day-start up to
// AsiaEndHour (or "now" if the Asian session hasn't finished yet today)
// for the session's high/low. Returns false if there isn't enough M15
...
// price.
bool DataHub_AsianRange(string symbol, int asiaEndHour, double &outHigh, double &outLow)
{
   datetime now       = TimeCurrent();
   datetime dayStart   = Session_DayStart(now);
   datetime asiaEndT   = dayStart + asiaEndHour * 3600;
   datetime scanEnd     = (asiaEndT < now) ? asiaEndT : now;
